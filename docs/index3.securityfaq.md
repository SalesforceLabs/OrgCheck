...
- The CLI plugin (`@orgcheck/sfdx-plugin`) uses the same Org Check API package and runs locally on the operator machine.
- It connects directly to your target org through the authenticated Salesforce CLI session and does not send data to Org Check external services.
- Generated reports (`csv`, `json`, `xlsx`) are written only to the local filesystem path provided by the user.
