...
export class OrgCheckDataFactory {
     ...
     * @private
     */
    private_allValidations;
    
    /**
     * @description List of all classes that needs dependencies
     * @type {Array<any>}
     * @private
     */
    private_needDependencies;
    ...
    constructor(sfdcManager) {
        ...
        Object.freeze(this.private_allValidations); 

        this.private_needDependencies = [
        ...
        if (this.private_needDependencies.every((dc) => IS_CLASS_EXTENDS(dc, OrgCheckData) === false)) {
            throw new TypeError(`The list of classes that need Dependencies must be of type 'array' with only OrgCheckData items.`);
        }
        Object.freeze(this.private_needDependencies); 

        this.private_instances = new Map();
    }
    ...
    getValidationRule(id) {
        return this.private_allValidations[id];
    }

    /**
     * @description Get the instance of the factiry for a given data class
     * @param {any} dataClass 
     * @returns {OrgCheckDataFactoryInstance}
     * @throws if the given dataClass is not an instance of OrgCheckData or OrgCheckInnerData
     * @public
     */
    getInstance(dataClass) {
        ...
        if (this.private_instances.has(dataClass) === false) {
            this.private_instances.set(dataClass, new OrgCheckDataFactoryInstance(
                dataClass, 
                isDataClassExtendsData ? this.private_allValidations.filter(v => v.applicable.includes(dataClass)) : [], 
                isDataClassExtendsData ? this.private_needDependencies.includes(dataClass) : false
            ));
        }
        // Return the instance
        return this.private_instances.get(dataClass);
    }
}
 ...
 * @public
 */
export class OrgCheckDataFactoryInstance {
    ...
    constructor(dataClass, validations, isDependenciesNeeded) {
        this.private_dataClass = dataClass;
        this.private_validations = validations;
        this.private_isDependenciesNeeded = isDependenciesNeeded;
    }
    ...
    create(configuration) {
        ...
        const row = new this.private_dataClass();
        ...
        if (this.private_validations.length > 0) {
            row.score = 0;
            row.badFields = [];
            row.badReasonIds = [];
        }
        // If dependencies are needed...
        if (this.private_isDependenciesNeeded === true && configuration.dependencies) {
     ...
     */
    computeScore(row) { 
        this.private_validations.filter(v => { 
            try { 
