...
export class RecipeWebLinks extends Recipe {
    ...
    async transform(data, logger, parameters) {
        ...
        const array = [];
        await Processor.forEach(weblinks, (/** @type {SFDC_WebLink} */weblink) => {
            // Augment data
            ...
            // Filter data
            if ((namespace === '*' || weblink.package === namespace) &&
                (objecttype === '*' || weblink.objectRef?.typeRef?.id === objecttype) &&
                (object === '*' || weblink.objectRef?.apiname === object)) {
                array.push(weblink);
            }
        });
