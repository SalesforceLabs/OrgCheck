 ...
 * @private
 */
const MAX_IDS_IN_DAPI_REQUEST_SIZE: number = 100;
 ...
 * @private
 */
const MAX_DAPI_TOOLING_BATCH_SIZE: number = 500;
 ...
 * @private
 */
const MAX_ATSCALE_TOOLING_BATCH_SIZE: number = 1000;
 ...
 * @private
 */
const MAX_NOQUERYMORE_BATCH_SIZE: number = 2000;
 ...
 * @private
 */
const MAX_STANDARDSOQL_BATCH_SIZE: number = 2000;

/**
 * @description Maximum number of members we want per type/request 
 * @private
 */
const MAX_MEMBERS_IN_METADATAAPI_REQUEST_SIZE: number = 10;
...
export class SalesforceManager implements SalesforceManagerIntf {
    ...
    public async soqlQuery(queries: SalesforceQueryRequest[], logger: SimpleLoggerIntf): Promise<Record<string, unknown>[][]> {
        ...
        const updateLogInformation = () => {
            logger?.log(`Processing ${queries?.length} SOQLs, 🧮 Records=${nbRecords}, 📣 QueryMoreCalls=${nbQueryMore}, 🏃‍♀️‍➡️ Pending=${pendingEntities?.size}, ✅ Done=${doneEntities?.size}, ❌ Error=${errorEntities?.size}...`);
        }
