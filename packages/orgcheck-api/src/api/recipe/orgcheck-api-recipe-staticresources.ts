...
export class RecipeStaticResources implements ServedRecipe<SfdcStaticResource[], Table> {
     ...
     * @public
     */
    public readonly title: string = '🗿 Static Resources';
     ...
     * @returns {Promise<SfdcStaticResource[]>} Returns the mixture
     * @async
     * @public
     */
    public async mix(ingredients: Map<string, unknown>, _logger: SimpleLoggerIntf, parameters: Map<string, string>): Promise<SfdcStaticResource[]> {
        ...
        const array: SfdcStaticResource[] = [];
        await MediumProcessor.forEach(staticResources, async (staticResource: SfdcStaticResource) => {            
            // Filter data
     ...
     * @param {SfdcStaticResource[]} mixture - Mixed data to be served to a table
     * @returns {Promise<Table>} The processed view
     * @async
     * @public
     */
    public async serveToTable(mixture: SfdcStaticResource[]): Promise<Table> {
        return TableFactory.create(this.title, new StaticResourcesTableDefinition(), mixture);
    }
