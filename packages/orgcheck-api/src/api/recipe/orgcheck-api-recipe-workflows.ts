...
export class RecipeWorkflows implements ServedRecipe<SfdcWorkflow[], Table> {
     ...
     * @public
     */
    public readonly title: string = '🚗 Workflows';
     ...
     * @param {SimpleLoggerIntf} _logger - Logger
     * @returns {Promise<SfdcWorkflow[]>} Returns the mixture
     * @async
     * @public
     */
    public async mix(ingredients: Map<string, any>, _logger: SimpleLoggerIntf): Promise<SfdcWorkflow[]> {

        // Get data
        const workflows: Map<string, SfdcWorkflow> = ingredients.get(DatasetAliases.WORKFLOWS);
     ...
     * @param {SfdcWorkflow[]} mixture - Mixed data to be served to a table
     * @returns {Promise<Table>} The processed view
     * @async
     * @public
     */
    public async serveToTable(mixture: SfdcWorkflow[]): Promise<Table> {
        return TableFactory.create(this.title, new WorkflowsTableDefinition(), mixture);
    }
