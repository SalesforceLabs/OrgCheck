...
export class TableFactory {
     ...
     * @param {Record<string, unknown>[]} records - The list of records to display in the table
     * @returns {Table} The created table instance
     * @static
     * @public
     */
    public static create(title: string, tableDefinition: TableDefinition, records: unknown[]): Table {
        let nbBadRows = 0;
        const rows = RowsFactory.create(
            tableDefinition, 
            records, 
            (_row: unknown, isRowBad: boolean) => { if (isRowBad) nbBadRows++ }, 
            () => {}
        );
        return {
            name: title,
    ...
    public static sort(table: Table, columnIndex: number, order: SortOrder): void {
        ...
        table.rows.sort((row1, row2) => {
            const cell1 = row1.cells[columnIndex];
            const cell2 = row2.cells[columnIndex];
            const data1 = cell1?.['data'] as Record<string, unknown> | undefined;
            const data2 = cell2?.['data'] as Record<string, unknown> | undefined;
            if (isIterative === true) {
                value1 = (data1?.['values'] as unknown[] | undefined)?.length || 0;
                value2 = (data2?.['values'] as unknown[] | undefined)?.length || 0;
            } else {
                value1 = data1?.[property];
                value2 = data2?.[property];
            }
            if (value1 && typeof value1 === 'string') value1 = value1.toUpperCase();
            if (value2 && typeof value2 === 'string') value2 = value2.toUpperCase();
            if (!value1 && value1 !== 0) return iOrder;
            if (!value2 && value2 !== 0) return -iOrder;
            return ((value1 as string | number) < (value2 as string | number) ? -iOrder : iOrder);
        }).forEach((row) => { 
        ...
        table?.rows?.forEach((row) => {
            ...
            row.cells?.forEach((cell) => {
                ...
                if (cell?.['typeofindex'] === true) { // for INDEX typed cell, we set the row's index
                    ...
                    exportRow.push(`${cellData?.['label']}`, `${cellData?.['value']}`);
                    ...
                    exportRow.push(
                        ARRAY_TO_STRING((cellData?.['values'] as Record<string, unknown>[] | undefined)?.map(v => `${(v?.['data'] as Record<string, unknown> | undefined)?.['label']}`)), 
                        ARRAY_TO_STRING((cellData?.['values'] as Record<string, unknown>[] | undefined)?.map(v => `${(v?.['data'] as Record<string, unknown> | undefined)?.['value']}`))
                    );
                    ...
                    const depValue = cellData?.['value'] as Record<string, unknown[]> | undefined;
                    exportRow.push(
                        ARRAY_TO_STRING((depValue?.['using'] as Record<string, unknown>[] | undefined)?.map(v => `${v?.['name']} (${v?.['type']} - ${v?.['id']})`)),
                        ARRAY_TO_STRING((depValue?.['referenced'] as Record<string, unknown>[] | undefined)?.map(v => `${v?.['name']} (${v?.['type']} - ${v?.['id']})`))
                    );
                    ...
                    exportRow.push(ARRAY_TO_STRING((cellData?.['values'] as Record<string, unknown>[] | undefined)?.map(v => `${v?.['data']}`)));
                } else { // for any other type use data?.value
                    exportRow.push(`${cellData?.['value'] ?? ''}`);
                }
            });
...
const STRING_MATCHER = (value: unknown, searchingValue: string) => {
    return `${value}`.toUpperCase().indexOf(searchingValue) >= 0;
}

const ARRAY_MATCHER = (array: Record<string, unknown>[], s: string) => {
    return array.findIndex((item) => {
        return Object.values(item?.['data'] as Record<string, unknown> ?? {}).findIndex((property) => {
            if (Array.isArray(property)) {
                return ARRAY_MATCHER(property as Record<string, unknown>[], s);
            }
...
const ARRAY_TO_STRING = (array: unknown[] | undefined) => {
