...
interface CheckResult {
    ...
    dateCheck: string;
    action: string;
    result: orgcheck.Data | orgcheck.Data[] | orgcheck.DataMatrixIntf | Map<string, boolean> | orgcheck.DataCollectionStatisticsIntf[];
}
...
export abstract class OrgCheckSfPluginAbstractCommand extends SfCommand<CheckResult> {
    ...
    public async run(): Promise<CheckResult> {
        ...
        const loggerSetup: OrgCheckSfPluginLoggerSetup = new OrgCheckSfPluginLoggerSetup(await Logger.child('OrgCheckSfPluginCommand'));
        ...
        if (flags['xlsx-file'] !== undefined) {
            // Export XSLX logic here
            writeFileSync(
                flags['xlsx-file'], 
                Buffer.from(orgcheck.TableUtils.exportAsXls(doggyBag)), 
                { flag: 'w' }
            );
        }
